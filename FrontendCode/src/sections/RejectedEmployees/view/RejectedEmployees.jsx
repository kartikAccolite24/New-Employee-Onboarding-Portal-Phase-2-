...
const RejectedEmployees = () => {
  ...
  const fetchUsersData = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.get("/getAllRejectedEmployees");
        ...
        renderCell: ({ row }) => (
          <>
            {(row.isApproved === "NOT_APPROVED" || row.isApproved === "REJECTED") ? (
              <Typography style={{ color: "red", fontFamily:"poppins" , marginTop:'15%', marginLeft:'-7%', fontSize:'14px'}}>Rejected</Typography>
            ) : (
