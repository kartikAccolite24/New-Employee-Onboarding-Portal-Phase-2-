...
const UnderReviewApplication = () => {
  ...
  const fetchUsersData = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.get("/getAllUnderReview");
  ...
  const filteredRows = usersData?.filter((user) =>
    ...
    const columns = [
      ...
      },
      {
        ...
        renderCell: ({ row }) => (
          <>
            {(row.isApproved === "NOT_APPROVED") ? (
              <Typography style={{ color: "blue", fontFamily:"poppins" , marginTop:'15%', marginLeft:'-7%', fontSize:'14px'}}>Under Review</Typography>
            ) : (
              <Typography style={{ color: "green", fontFamily:"poppins" , marginTop:'15%', marginLeft:'-7%', fontSize:'14px'}}>Under Review</Typography>
            )}
          </>
        )
      },
